...
#  before_filter :authenticate_usuario!
  ...
  def index
    id = params[:id]
    if id.nil?
      @persona = Persona.find_by_id(usuario_id)
    else
      if @persona = Persona.find_by_id(id)
  ...
  def show
    @persona = Persona.find_by_id(usuario_id)
    vinculo = Vinculo.find_by_id(params[:id])   # encuentra el vinculo en la superclase
    modelo = vinculo.type.constantize           # Converteer el tipo del vinculo en la clase del modelo 
    @vinculo = modelo.find_by_id(vinculo.id)    # Inicia el vinculo correcto, aprovecha la MT Inheritance
  ...
  def new
    @persona = Persona.find_by_id(usuario_id)       # Inicia la persona
    tipo_vinculos = params[:vinculos]
    model = tipo_vinculos.classify.constantize      # Converteer el tipo del vinculo en la clase del model
    @vinculo = model.new
  ...
  def edit
    @persona = Persona.find_by_id(usuario_id)
    vinculo = Vinculo.find_by_id(params[:id])
  ...
  def create
    ...
    @vinculo = modelo.new(params[:vinculo])
    respond_to do |format|
      if @vinculo.save
  ...
  def update
    ...
    vinculo = Vinculo.find_by_id(params[:id])
    modelo = vinculo.type.constantize
    @vinculo = modelo.find_by_id(vinculo.id)
    respond_to do |format|
